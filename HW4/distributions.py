...
class Normal(dist.Normal):
    ...
    def grad_log_prob(self, c):

        # Zero grads
        for param in self.Parameters():
            param.grad = torch.tensor(0, dtype=torch.float32)

        log_prob = self.log_prob(c)
        log_prob.backward()

        grads = torch.stack([param.grad.clone() for param in self.Parameters()])
...
class Categorical(dist.Categorical):
    ...
    def grad_log_prob(self, c):
        ...
        # Zero grads
        for param in self.Parameters():
            param.grad = torch.tensor([0] * param.size(0), dtype=torch.float32)
...
class Dirichlet(dist.Dirichlet):
    ...
    def __init__(self, concentration=torch.tensor([1.0, 1.0, 1.0])):
...
class Gamma(dist.Gamma):
    ...
    def __init__(self, concentration=torch.tensor(2.0), rate=torch.tensor(2.0)):
        if rate > 20.:
...
if __name__ == '__main__':
    ...
    # How to use this, given some input tensors that don't necessarily have gradients enables
